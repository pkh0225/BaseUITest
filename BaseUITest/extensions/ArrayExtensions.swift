...
extension Array {
    static func sw_button(_ buttonA: UIButton?, isEqualTo buttonB: UIButton?) -> Bool {
        guard let buttonA = buttonA, let buttonB = buttonB else { return false }

        let backgroundColorA: UIColor? = buttonA.backgroundColor
        let backgroundColorB: UIColor? = buttonB.backgroundColor
        let haveEqualBackgroundColors: Bool = (backgroundColorA == nil && backgroundColorB == nil) || backgroundColorA?.isEqual(backgroundColorB) ?? false
        let titleA: String? = buttonA.title(for: .normal)
        let titleB: String? = buttonB.title(for: .normal)
        let haveEqualTitles: Bool = (titleA == nil && titleB == nil) || (titleA == titleB)
        let normalIconA: UIImage? = buttonA.image(for: .normal)
        let normalIconB: UIImage? = buttonB.image(for: .normal)
        let haveEqualNormalIcons: Bool = (normalIconA == nil && normalIconB == nil) || normalIconA?.isEqual(normalIconB) ?? false
        let selectedIconA: UIImage? = buttonA.image(for: .selected)
        let selectedIconB: UIImage? = buttonB.image(for: .selected)
        let haveEqualSelectedIcons: Bool = (selectedIconA == nil && selectedIconB == nil) || selectedIconA?.isEqual(selectedIconB) ?? false
        return haveEqualBackgroundColors && haveEqualTitles && haveEqualNormalIcons && haveEqualSelectedIcons
    }

    func sw_isEqual(toButtons buttons: [Any]?) -> Bool {
        guard let buttons = buttons, buttons.count == self.count else { return false }

        for idx in 0..<count {
            let buttonA = self[idx]
            let buttonB = buttons[idx]

            if let buttonA = buttonA as? UIButton, let buttonB = buttonB as? UIButton {
                if !Array.sw_button(buttonA, isEqualTo: buttonB) {
                    return false
                }
            }
            else {
...
extension Array where Element: UIView {
    ...
    @inline(__always) public func sizeThatFits(maxLineCount: Int, spacing: (inset: UIEdgeInsets, columnSpacing: CGFloat, lineSpacing: CGFloat), fit: (width: Bool, height: Bool)) -> CGSize {
        ...
        if fit.width {
            if superView.translatesAutoresizingMaskIntoConstraints == false, superView.isWidthConstraint {
                superView.widthConstraint = contentSize.width
            }
        ...
        if fit.height {
            if superView.translatesAutoresizingMaskIntoConstraints == false, superView.isHeightConstraint {
                superView.heightConstraint = contentSize.height
            }
    ...
    @inline(__always) public func sizeThatFitsVertical(maxLineCount: Int, spacing: (inset: UIEdgeInsets, lineSpacing: CGFloat), fit: (width: Bool, height: Bool)) -> CGSize {
        ...
        if fit.width {
            ...
            if superView.isWidthConstraint {
                superView.widthConstraint = contentSize.width
            }
        }
        if fit.height {
            ...
            if superView.isHeightConstraint {
                superView.heightConstraint = contentSize.height
            }
        }
